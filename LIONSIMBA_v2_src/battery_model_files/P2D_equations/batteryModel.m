...
% BATTERYMODEL retruns the set of residuals of the complete battery model.
...
for i=1:n_cells
    ...
    if param.OperatingMode==1 || param.OperatingMode==4
        param.I_density = param_tot{1}.getCurrentDensity(t,t0,tf,param_tot{i}.extraData); %param.I_density gets updated every time-step through this important line of code, i.e. fetch the value (either constant or from external function file) at time 't'
    ...
    if(param.TemperatureEnabled==1)
        ...
        cs_star_avg_pos = sum(cs_star(1:param.Np))/param.Np;        % (scalar) average surface conc in positive electrode
        cs_star_avg_neg = sum(cs_star(param.Np+1:end))/param.Nn;    % (scalar) average surface conc in negative electrode
        ...
        % [resdT, rhsT] = thermalModel(ce,Phie,Phis,Keff,jflux + [zeros(param.Np,1);J_S],T,dT,Up,Un,dudt_p,dudt_n,param,I_density);
        [resdT, rhsT] = thermalModel(ce,cs_star_avg,Phis,Keff,jflux + [zeros(param.Np,1);J_S],T,dT,Up,Un,dudt_p,dudt_n,param,I_density);
    else
