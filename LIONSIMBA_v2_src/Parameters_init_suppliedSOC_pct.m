function param = Parameters_init_suppliedSOC_pct(init_cell_soc_percent) % Cell SOC [%] to begin simulation with
% PARAMETERS_INIT_SUPPLIEDSOC_PCT Defines the parameters used in simulation. To change the cell/simulator parameters, modify the values here.
% The user provides a starting SOC (%) for beginning the simulation.
...
% Cutoff voltage [V]
param.CutoffVoltage = 2.5;  % Adjusted to new values in code elsewhere for acceleration/fast charge

% Cutover voltage [V]
param.CutoverVoltage = 4.3; % Adjusted to new values in code elsewhere for acceleration/fast charge

% Cutoff SOC [%]
param.CutoffSOC = 0; % Adjusted to new values in code elsewhere for acceleration/fast charge

% Cutover SOC [%]
param.CutoverSOC = 100; % Adjusted to new values in code elsewhere for acceleration/fast charge
...
param.Nal   = 10; % Not relevant if using a lumped thermal model. % Adjusted to new values in code elsewhere for acceleration/fast charge

% Number of control volumes used for discretising the positive electrode domain in the axial (through-thickness) direction
param.Np    = 10;   % Adjusted to new values in code elsewhere for acceleration/fast charge

% Number of control volumes used for discretising the separator domain in the axial (through-thickness) direction
param.Ns    = 10;   % Adjusted to new values in code elsewhere for acceleration/fast charge

% Number of control volumes used for discretising the negative electrode domain in the axial (through-thickness) direction
param.Nn    = 10;   % Adjusted to new values in code elsewhere for acceleration/fast charge
...
param.Ncu   = 10; % Not relevant if using a lumped thermal model. % Adjusted to new values in code elsewhere for acceleration/fast charge
...
param.Nr_p = 10;    % Adjusted to new values in code elsewhere for acceleration/fast charge

% Number of control volume (shells) used for discretising each anode particle in the radial/spherical directi
param.Nr_n = 10;    % Adjusted to new values in code elsewhere for acceleration/fast charge
